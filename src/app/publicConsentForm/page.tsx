...
//         const consentName = params.ConsentName ? decodeURIComponent(params.ConsentName) : "WAIVER FORM";
...
const ConsentForm: React.FC<ConsentFormProps> = () => {
    ...
    const generatePDFFile = async (): Promise<File | null> => {
        ...
        try {
            ...
            const pdfBlob = pdf.output('blob');
            const timestamp = new Date().toISOString().replace(/[:.]/g, '-').substring(0, 19);
            const safeName = (formData.patientName || 'participant').replace(/[^a-zA-Z0-9]/g, '_');
            const safeTitle = consentName.replace(/[^a-zA-Z0-9]/g, '_'); // FIXED: Use consentName
            const filename = `${safeTitle}_${safeName}_${timestamp}.pdf`;
    ...
    const createPayload = async () => {
        try {
            ...
            const payload = {
                ...
                participantInfo: {
                    consentId: consentId, // FIXED: Use consentId
                    name: formData.patientName || profileData?.fullName,
                    guardianName: formData.guardianName,
                ...
                waiverDetails: {
                    programType: "HIGH5 Sports Performance",
                    organizationName: 'HIGH5 PERFORMANCE',
                    waiverAccepted: true,
                    risks_acknowledged: [
                        'Cardiovascular conditioning risks',
                        'Muscle strength and endurance exercise risks',
                        'Flexibility training risks',
                        'Physical activity participation risks',
                        'Program duration and commitment requirements',
                        'Touch training techniques',
                        'Payment obligations',
                        'Injury liability acknowledgment'
                    ]
                },
                metadata: {
                    userAgent: navigator.userAgent,
    ...
    const handleVerify = async (): Promise<void> => {
        ...
        try {
            setIsGeneratingPDF(true);
            const payload = await createPayload();
            console.group('📋 WAIVER FORM SUBMISSION DATA');
            console.groupEnd();
            const pdfFile = await generatePDFFile();
            if (!pdfFile) {
                throw new Error('Failed to generate PDF file');
                    ...
                    <div ref={formRef} className="mb-4 bg-white mx-auto">
                        ...
                        <div className="space-y-4 mx-4 mb-6">
                            ...
                            <div className="flex items-center gap-2">
                                <label className="text-sm font-medium text-black whitespace-nowrap min-w-fit">
                                ...
                                <div className="flex-grow border-b border-gray-400">
                                    <input
                                        ...
                                        onChange={handleInputChange}
                                        placeholder=""
                                        className="w-full p-1 border-none outline-none bg-transparent pdf-hidden"
                                        required
                                    />
                                    <div className="p-1 min-h-6 flex items-center pdf-show font-medium" style={{ display: 'none' }}>
                                        {formData.patientName || profileData?.fullName}
                                    </div>
                                </div>
                            </div>

                            {/* Phone, Sport, Age Fields */}
                            <div className="flex items-center gap-4">
                        ...
                        <div className="mx-4 mb-8">
                            <div className="mb-4 text-sm text-black font-medium">
                            ...
                            <div className="space-y-4 text-sm text-black">
                                <div className="flex gap-2">
                                    ...
                                    <span>My consent to perform cardiovascular conditioning, and/or muscle strength & endurance exercises, and/or flexibility training offered by High5 Sports Performance</span>
                                </div>

                                <div className="flex gap-2">
                                    <span className="font-bold min-w-fit">2)</span>
                                    <span>My consent to answer questions concerning my physical activity participation and lifestyle; my understanding that there are potential risks to participate in this program, and that I may stop or delay physical activity if I so desire</span>
                                </div>

                                <div className="flex gap-2">
                                    <span className="font-bold min-w-fit">3)</span>
                                    <span>My obligation to immediately inform the instructor of any pain, discomfort, fatigue, or any other symptoms that I may suffer before, during and after a fitness session</span>
                                </div>

                                <div className="flex gap-2">
                                    <span className="font-bold min-w-fit">4)</span>
                                    <span>My understanding that I may stop or delay physical activity if I so desire and that the instructor may terminate activity upon observation of any symptoms of unusual discomfort or distress</span>
                                </div>

                                <div className="flex gap-2">
                                    <span className="font-bold min-w-fit">5)</span>
                                    <span>My understanding that I may ask any questions or request further explanation or information about the activities or procedures at any time before, during or after completing a fitness session</span>
                                </div>

                                <div className="flex gap-2">
                                    <span className="font-bold min-w-fit">6)</span>
                                    <span>My understanding that my instructor(s) may use touch training to correct alignment and/or to focus my concentration on a particular muscle area being targeted. If I feel uncomfortable or experience any type of discomfort with touch training, I will immediately request that my instructor(s) discontinue using this technique</span>
                                </div>

                                <div className="flex gap-2">
                                    <span className="font-bold min-w-fit">7)</span>
                                    <span>My understanding that payment for High5 Sports Performance programs occurs <u>prior</u> to program participation. I understand all fitness classes and personal training sessions are non-transferable and non-refundable</span>
                                </div>

                                <div className="flex gap-2">
                                    <span className="font-bold min-w-fit">8)</span>
                                    <span>I hereby release and hold harmless High5 Sports Performance and its agents, employees, contractors, officers, and volunteers from and against all claims, actions, costs, expenses (including financial compensation), damages, demands, or proceedings in respect of any loss or damage, howsoever caused, arising out of or in connection with this program and my participation in the program.</span>
                                </div>

                                <div className="flex gap-2">
                                    <span className="font-bold min-w-fit">9)</span>
                                    <span>I state that I have seen the demo class and that I am physically fit to undergo training as shown to me in the demo class. I state that I have disclosed all information related to previous injuries, if any, in the box below.</span>
                                </div>

                                <div className="flex gap-2">
                                    <span className="font-bold min-w-fit">10)</span>
                                    <span>I state that I am aware that no instructor or High5 Sports Performance can be held liable for any injury or even death during the course of training and that I am willfully undergoing such training at my own risk.</span>
                                </div>
                            </div>
                        </div>

                        {/* Signature Section */}
                        <div className="space-y-6 pt-6 mx-4 mb-6">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <div className="w-32 text-black font-medium">
                            ...
                            <div className="flex items-center gap-4">
                                <label className="text-sm font-medium text-black whitespace-nowrap min-w-fit">
                                    Parent/Guardian signature (In case of Minor)
                                </label>
                                <div className="flex-grow border-b border-gray-400 min-h-6">
                                </div>
                            </div>

                            {/* Digital Signature Display for PDF */}
                            {formData.signatureFile && imagePreview && (
                                <div className="flex items-center gap-4 mt-4 pdf-show" style={{ display: 'none' }}>
                                    <label className="text-sm font-medium text-black whitespace-nowrap">
                                        Digital Signature:
